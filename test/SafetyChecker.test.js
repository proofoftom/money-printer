...
describe("SafetyChecker", () => {
  ...
  beforeEach(() => {
    ...
    mockMarketData = {
      ...
      minted: Date.now() - 3700000, // 1 hour and 100 seconds ago
      currentPrice: 1,
      initialPrice: 0.5,
      priceVolatility: 30,
      uniqueBuyers: 60,
  ...
  describe("Market Cap Checks", () => {
    it("should reject if market cap is too high", () => {
      mockMarketData.marketCapSol =
        config.SAFETY.MAX_MARKET_CAP_USD / 100 + 100;
    ...
    it("should accept valid market cap", () => {
      mockMarketData.marketCapSol =
        (config.SAFETY.MIN_MARKET_CAP_USD + 1000) / 100;
  ...
  describe("Time and Age Checks", () => {
    it("should reject if token is too new", () => {
      mockMarketData.minted =
        Date.now() - config.SAFETY.MIN_TOKEN_AGE_SECONDS * 500;
      assert.strictEqual(safetyChecker.checkTokenAge(mockMarketData), false);
    });

    it("should accept token with valid age", () => {
      mockMarketData.minted =
        Date.now() - config.SAFETY.MIN_TOKEN_AGE_SECONDS * 2000;
  ...
  describe("Price Action Checks", () => {
    it("should reject if price pump is too high", () => {
      mockMarketData.currentPrice =
        mockMarketData.initialPrice * (config.SAFETY.MAX_PUMP_MULTIPLE + 1);
  ...
  describe("Volume Pattern Checks", () => {
    it("should reject if volume-price correlation is too low", () => {
      mockMarketData.volumePriceCorrelation =
        config.SAFETY.MIN_VOLUME_PRICE_CORRELATION - 0.2;
      assert.strictEqual(
        safetyChecker.checkVolumePatterns(mockMarketData),
        false
      );
    });

    it("should reject if wash trading percentage is too high", () => {
      mockMarketData.suspectedWashTradePercentage =
        config.SAFETY.MAX_WASH_TRADE_PERCENTAGE + 5;
      assert.strictEqual(
        safetyChecker.checkVolumePatterns(mockMarketData),
        false
      );
