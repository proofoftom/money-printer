...
class SafetyChecker {
  ...
  getTradingPatternRejectionReason(token) {
    if (token.uniqueBuyers < config.SAFETY.MIN_UNIQUE_BUYERS)
      return "insufficientBuyers";
    if (
  ...
  checkHolderDistribution(token) {
    ...
    if (token.minHolderWalletAge < config.SAFETY.MIN_HOLDER_WALLET_AGE)
      return false;
    return true;
  }
